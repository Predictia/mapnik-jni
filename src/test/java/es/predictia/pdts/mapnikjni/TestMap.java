...
public class TestMap {
    ...
    public void testCollectAttributes() {
        ...
        // Load from resources, and copy to a temporary file
        var mapFile = safeMapFile("testmap.xml");
        if (mapFile == null) {
            System.out.println("Unable to create temporary file");
            return;
        }
        map.loadMap(mapFile.toString(), false);
        ...
        assertArrayEquals(attrs, new String[]{"landuse", "leisure", "name", "way_area"});
    ...
    private Path safeMapFile(String s) {
        // Create a temporary file
        try {
            var path = Files.createTempFile("testmap", ".xml");
            // Copy the resource to the temporary file
            try (var is = getClass().getClassLoader().getResourceAsStream(s)) {
                if (is == null) {
                    return null;
                }
